	...
	class WP_MANGA_RECENT extends WP_Widget {
		...
		function widget( $args, $instance ) {
			...
			if(class_exists('WP_MANGA_CACHE_SYSTEM')){
				$results = WP_MANGA_CACHE_SYSTEM::get_cached_or_query($query_args);
				$posts_count = WP_MANGA_CACHE_SYSTEM::get_cached_or_count($query_args);
			} else {
            ...
            <div class="c-widget-content <?php echo esc_attr($style); ?> <?php echo $button != '' ? 'with-button' : ''; ?>">
				<?php
					...
					$wpmanga_recent_manga_ids = array_map(function($item){ return $item->ID;}, $results);
