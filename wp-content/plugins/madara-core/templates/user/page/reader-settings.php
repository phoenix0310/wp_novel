<?php
	...
	if ( isset( $_POST['reader-settings-submit'] ) ) {
		update_user_meta( $user_id, '_manga_reading_style', sanitize_text_field($reading_style) );
		update_user_meta( $user_id, '_manga_img_per_page', sanitize_text_field($img_per_page) );
		update_user_meta( $user_id, '_manga_user_site_schema', sanitize_text_field($_POST['_manga_site_schema']) );
		$is_update = true;
	}
