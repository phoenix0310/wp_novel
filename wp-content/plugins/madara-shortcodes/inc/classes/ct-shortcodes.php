	...
	class CT_Shortcodes {
		...
		function init() {
			...
			if ( is_admin() ) {
				...
				wp_enqueue_script( 'ct_shortcode_admin', CT_SHORTCODE_PLUGIN_URL . 'assets/js/admin.js', array('jquery'), '1.6.2' );

				// SECURITY PATCH: Add nonce for AJAX calls
				wp_localize_script( 'ct_shortcode_admin', 'ct_ajax_object', array(
					'ajax_url' => admin_url( 'admin-ajax.php' ),
					'nonce'    => wp_create_nonce( 'ct_save_widget_text_nonce' )
				) );
		...
		function madara_shortcodes_wp_head() {
			...
			if ( $custom_css != '' ) {
				echo '<style type="text/css" id="ct_global_custom_css">' . esc_html( $custom_css ) . '</style>';
			}
			...
			if ( $ot_custom_css != '' ) {
				echo '<style type="text/css" id="ct_global_ot_custom_css">' . esc_html( $ot_custom_css ) . '</style>';
			}
		...
		function widget_text_save_callback() {
			...
			check_ajax_referer( 'ct_save_widget_text_nonce', 'security' );

			// Verify user has permission to edit theme
			...
			foreach ( $vals as $item ) {
				...
				if ( endsWith( $key, '[text]' ) ) {
					...
					$css = madara_sanitize_custom_css( $css );
