...
jQuery(document).ready(function (e) {
    ...
    if(jQuery('body').hasClass('post-type-page')){
		...
		if (madara_admin_vars.template_name == 'page-templates/front-page.php') {
			// do nothing
			front_page_meta_boxes.removeClass('hide');
		} else {
			front_page_meta_boxes.addClass('hidden');
		}

		if (madara_admin_vars.template_name == 'page-templates/fullpage.php') {
	...
	jQuery(window).on("wp_manga_content_chapter_save", function (e, chapterData) {
		var ads_link = jQuery('.wp-manga-modal-body #chapter-ads-link').val();
		var obj = '';
		if (ads_link !== '' && typeof ads_link !== 'undefined') {
			obj = jQuery.parseJSON('{ "ads_link": "' + ads_link + '" }');
			chapterData.chapterCreatingTriggerData = chapterData.chapterCreatingTriggerData ? JSON.stringify({...obj, ...jQuery.parseJSON(chapterData.chapterCreatingTriggerData)}) : JSON.stringify(obj);
			window.mangaChapterSaveTrigger = chapterData;
		}
	});

	jQuery(window).on("wp_manga_after_admin_fill_chapter_modal_content", function (e, triggerData) {
		jQuery('input#chapter-ads-link').val(triggerData.ads_link);
	});
