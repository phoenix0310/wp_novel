...
if ( class_exists( 'WP_Customize_Control' ) ) {
	...
	class Skyrocket_Custom_Control extends WP_Customize_Control {
		protected $customControlsCssVersion = '1.3';
		protected $skyrocketCustomControlsJsVersion = '1.8';
		protected function get_resource_url() {
			if( strpos( wp_normalize_path( __DIR__ ), wp_normalize_path( WP_PLUGIN_DIR ) ) === 0 ) {
				// We're in a plugin directory and need to determine the url accordingly.
				return plugin_dir_url( __DIR__ ) . 'customizer/';
			}

			return trailingslashit( get_template_directory_uri() ) . 'app/customizer/';
		}
	}
	...
	class Skyrocket_Slider_Custom_Control extends Skyrocket_Custom_Control {
		 ...
		 */
		public function enqueue() {
			wp_enqueue_script( 'madara-custom-controls-js', $this->get_resource_url() . 'js/customizer.js', array( 'jquery', 'jquery-ui-core' ), $this->skyrocketCustomControlsJsVersion, true );
			wp_enqueue_style( 'madara-custom-controls-css', $this->get_resource_url() . 'css/customizer.css', array(), $this->customControlsCssVersion, 'all' );
		}
		/**
		 * Render the control in the customizer
	...
	class Skyrocket_Sortable_Repeater_Custom_Control extends Skyrocket_Custom_Control {
		...
		public function __construct( $manager, $id, $args = array(), $options = array() ) {
			...
			$this->inputs = wp_parse_args( $this->inputs,
				array('default' => ['type' => 'url', 'label' => 'URL', 'placeholder' => 'https://'])
			);
		}
		/**
		 * Enqueue our scripts and styles
		...
		public function render_content() {
		?>
		  <div class="sortable_repeater_control" data-inputs='<?php echo json_encode($this->inputs);?>'>
				<?php if( !empty( $this->label ) ) { ?>
					<span class="customize-control-title"><?php echo esc_html( $this->label ); ?></span>
				<?php } ?>
				<?php if( !empty( $this->description ) ) { ?>
				...
				<div class="sortable_repeater sortable">
					<div class="repeater">
						<?php foreach($this->inputs as $key => $data){
							?>
							<span><?php echo $data['label'];?></span><input type="text" data-sanitize="<?php echo $data['type'];?>" value="" class="repeater-input repeater-input<?php echo ($key != 'default' ? '-' . $key : '');?>" placeholder="<?php echo $data['placeholder'];?>" /><br/>
							<?php
						}?>
	...
	class Skyrocket_Dropdown_Select2_Custom_Control extends Skyrocket_Custom_Control {
		 ...
		 */
		public function enqueue() {
			wp_enqueue_script( 'madara-select2-js', $this->get_resource_url() . 'js/select2.full.min.js', array( 'jquery' ), '4.0.13', true );
			wp_enqueue_script( 'madara-custom-controls-js', $this->get_resource_url() . 'js/customizer.js', array( 'madara-select2-js' ), $this->skyrocketCustomControlsJsVersion, true );
			wp_enqueue_style( 'madara-custom-controls-css', $this->get_resource_url() . 'css/customizer.css', array(), $this->customControlsCssVersion, 'all' );
			wp_enqueue_style( 'madara-select2-css', $this->get_resource_url() . 'css/select2.min.css', array(), '4.0.13', 'all' );
		}
		/**
		 * Render the control in the customizer
		 */
	...
	class Skyrocket_Customize_Alpha_Color_Control extends Skyrocket_Custom_Control {
		 ...
		 */
		public function enqueue() {
			wp_enqueue_script( 'madara-custom-controls-js', $this->get_resource_url() . 'js/customizer.js', array( 'jquery', 'wp-color-picker' ), $this->skyrocketCustomControlsJsVersion, true );
			wp_enqueue_style( 'madara-custom-controls-css', $this->get_resource_url() . 'css/customizer.css', array( 'wp-color-picker' ), $this->customControlsCssVersion, 'all' );
		}
		/**
		 * Render the control in the customizer
		 */
	...
	class Skyrocket_Alpha_Color_Control extends Skyrocket_Custom_Control {
		 ...
		 */
		public function enqueue() {
			wp_enqueue_script( 'wp-color-picker-alpha', $this->get_resource_url() . 'js/wp-color-picker-alpha-min.js', array( 'wp-color-picker' ), '3.0.2', true );
			wp_enqueue_script( 'madara-custom-controls-js', $this->get_resource_url() . 'js/customizer.js', array( 'jquery', 'wp-color-picker-alpha' ), $this->skyrocketCustomControlsJsVersion, true );
			wp_enqueue_style( 'madara-custom-controls-css', $this->get_resource_url() . 'css/customizer.css', array(), $this->customControlsCssVersion, 'all' );
